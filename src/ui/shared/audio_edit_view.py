...
class AudioEditView:
    def __init__(
        self, audio_name: str, audio_bytes: io.BytesIO, sr: int, state_prefix: str
    ):
    ...
    def render(self) -> None:
        ...
        speed_rate = col1.slider(
            ...
            min_value=0.5,
            max_value=2.0,
            value=cache.speed_rate,
            step=0.1,
            key=f"{self._state_prefix}_speed",
        )
        pitch_steps = col2.slider(
            ...
            step=1,
            key=f"{self._state_prefix}_pitch",
        )
        noise_amount = col3.slider(
            ...
            step=0.01,
            key=f"{self._state_prefix}_noise",
        )
        ...
        if st.button(
            ...
            key=f"{self._state_prefix}_apply_btn",
        ):
